...
export class BotonWhatsappComponent {
  ...
  enlaceWhatsapp = `https://wa.me/${WHATSAPP_NUMBER}`;

  private urlActual = signal<string>(this.router.url);
  public mostrar = computed(() => {
    const url = this.urlActual();
    return !RUTAS_OCULTAR.some((r) => url.startsWith(r));
  });

  constructor() {
    this.router.events
      .pipe(filter((e): e is NavigationEnd => e instanceof NavigationEnd))
      .subscribe((e) => this.urlActual.set(e.urlAfterRedirects || e.url));
  }
}
